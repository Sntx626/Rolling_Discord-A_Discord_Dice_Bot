...
async def shutdown(ctx):
    print_on_command_call(ctx.author, 'shutdown', '')
    await ctx.bot.logout()
    print_bot(f"shutdown...")

# load cogs
@client.command()
@commands.has_permissions(administrator=True)
async def load(ctx, extension):
    print_on_command_call(ctx.author, 'load', extension)
...
async def unload(ctx, extension):
    print_on_command_call(ctx.author, 'unload', extension)
    ...
    await ctx.send(embed=embed)
    print_bot(f"unloaded: '{extension}'")

# reload cogs
@client.command()
@commands.has_permissions(administrator=True)
async def reload(ctx, *, extension=''):
    print_on_command_call(ctx.author, 'reload', extension)
    if extension == '':
        ...
        await ctx.send(embed=embed)
        print_bot(f"reloaded: all", ctx.author, "reload", extension)
    else:
        client.unload_extension(f'cogs.{extension}')
        client.load_extension(f'cogs.{extension}')
        embed = discord.Embed(
        ...
        await ctx.send(embed=embed)
        print_bot(f"reloaded: '{extension}'")
