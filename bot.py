...
status = cycle([f"Listening to '{command_prefix}roll d20'",
                f"Listening to '{command_prefix}roll d4 6'",
                f"Listening to '{command_prefix}roll d100'",
                f"Listening to '{command_prefix}roll 12 20'",
                f"Listening to '{command_prefix}roll 4 40'",
                f"Listening to '{command_prefix}roll 8'",
                f"Listening to '{command_prefix}roll 6'",
                f"Listening to '{command_prefix}roll d4'"])
...
async def reload(ctx, *, extension=''):
    ...
    if extension == '':
        ...
        await ctx.send(embed=embed)
        tools.print_bot(f"reloaded: all")
    else:
        client.unload_extension(f'cogs.{extension}')
        client.load_extension(f'cogs.{extension}')
        embed = discord.Embed(
